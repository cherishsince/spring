...
/**
 * 用于在{@code TransactionSynchronizationManager}中保持当前的{@codesqlsession}。{@code SqlSessionFactory}
 * 它创建了{@codesqlsession}作为键。{@code ExecutorType}也被保留，以便能够检查
 * 用户试图在发送期间更改它（这是不允许的），并在这种情况下引发异常。
 *
